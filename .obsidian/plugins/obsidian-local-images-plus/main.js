...
class SettingTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        new obsidian.Setting(containerEl)
            .setName("Keep original image name from URL")
            .setDesc("Use the original filename from the image URL instead of generating an MD5 hash name. When enabled, this takes priority over MD5 naming for downloaded web images.")
            .addToggle((toggle) => toggle
            .setValue(this.plugin.settings.useOriginalImageName)
            .onChange((value) => __awaiter(this, void 0, void 0, function* () {
            this.plugin.settings.useOriginalImageName = value;
            yield this.plugin.saveSettings();
        })));
        new obsidian.Setting(containerEl)
                    ...
                    const { fileName, needWrite } = yield lock.acquire(match, function () {
                        return __awaiter(this, void 0, void 0, function* () {
                            ...
                            if (fileExt == "png" && settings.PngToJpeg) {
                                ...
                                else if (settings.ImgCompressionType == "image/png") {
                                    compType = "image/png";
                                }
...
function chooseFileName(adapter, dir, link, contentData, settings) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let needWrite = true;
        let fileName = "";
        let suggestedBaseName = baseName;
        if (settings.useOriginalImageName) {
            const urlPath = decodeURIComponent(parsedUrl.pathname);
            const originalName = path__default["default"].basename(urlPath, path__default["default"].extname(urlPath));
            if (originalName && originalName.length > 0) {
                suggestedBaseName = originalName;
            }
        }
        const suggestedName = pathJoin([dir, cleanFileName(`${suggestedBaseName}` + `.${fileExt}`)]);
        if (yield adapter.exists(suggestedName, false)) {
            ...
            else if (settings.useOriginalImageName) {
                let inc = 1;
                let candidateName = pathJoin([dir, cleanFileName(`${suggestedBaseName} (${inc})` + `.${fileExt}`)]);
                while (yield adapter.exists(candidateName, false)) {
                    inc++;
                    candidateName = pathJoin([dir, cleanFileName(`${suggestedBaseName} (${inc})` + `.${fileExt}`)]);
                }
                        ...
                        for (let el of embeds) {
                            ...
                            if ((this.newfCreated.indexOf(el.link) != -1 || (obsmdir != "" && (this.newfCreated.includes(oldpath) || this.newfCreated.includes(el.link)))) &&
                                ...
                                if (this.settings.PngToJpegLocal && fileExt == "png") {
                                    ...
                                    else if (this.settings.ImgCompressionType == "image/png") {
                                        compType = "image/png";
                                        compExt = ".png";
                                    }
